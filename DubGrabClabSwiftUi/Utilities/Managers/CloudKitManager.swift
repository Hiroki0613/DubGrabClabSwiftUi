...
//        operation.resultsLimit = 1
        ...
        operation.queryCompletionBlock = { cursor, error in
            ...
            if let cursor = cursor {
                print("1. Initial Cursor is not nil - \(cursor) ")
                print("Current Dictionary - \(checkedInProfiles) ")
                self.continueWithCheckedInProfileDict(cursor: cursor,
                                                      dictionary: checkedInProfiles) { result in
                    switch result {
                    case .success(let profiles):
                        print("1. Initial Cursur Success  - Dictionary - \(profiles) ")
                        completed(.success(profiles))
                    case .failure(let error):
                        print("1. Initial Error  - Dictionary - \(error) ")
...
//        operation.resultsLimit = 1
        ...
        operation.queryCompletionBlock = { cursor, error in
            ...
            if let cursor = cursor {
                print("2. Recursive Cursor is not nil - \(cursor) ")
                print("Current Dictionary - \(checkedInProfiles) ")
                self.continueWithCheckedInProfileDict(cursor: cursor,
                                                      dictionary: checkedInProfiles) { result in
                    switch result {
                    case .success(let profiles):
                        ...
                        completed(.success(profiles))
                    case .failure(let error):
                        print("Recursive Failure - Dictionary - \(error) ")
                        completed(.failure(error))
                    }
                }
            } else {
